@php
    ...
    $recent_orders = \Plugin\CartLooksCore\Models\Orders::with(['customer_info', 'guest_customer'])
        ->select('order_code', 'id', 'created_at', 'total_payable_amount', 'customer_id')
        ->orderBy('id', 'DESC')
        ->take(6)
        ->get();
    $top_customers = \Plugin\CartLooksCore\Models\Customers::withCount('orders')
        ->orderBy('orders_count', 'DESC')
        ->take(5)
        ->get();
    
    $top_selling_products = \Plugin\CartLooksCore\Models\Product::select(['id', 'name', 'permalink', 'thumbnail_image'])
        ->withCount(['orders'])
    ...
    $popular_products = \Plugin\CartLooksCore\Models\Product::select(['id', 'name', 'permalink', 'thumbnail_image'])
        ->withCount(['reviews'])
    ...
    $top_categories = $top_categories->map(function ($category, $key) {
        $category_details = \Plugin\CartLooksCore\Models\ProductCategory::select('id', 'name', 'permalink')
    ...
    $top_brands = $top_brands->map(function ($brand, $key) {
        $brand_details = \Plugin\CartLooksCore\Models\ProductBrand::select('id', 'name', 'permalink')
            ->where('id', $brand->id)
            ->first();
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($order->customer_info != null)
                                                <a
                                                    href="{{ route('plugin.cartlookscore.customers.details', ['id' => $order->customer_id]) }}">{{ $order->customer_info->name }}</a>
                                            @else
                        ...
                        @foreach ($top_customers as $customer)
                            <div
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="content">
                                        <a href="{{ route('plugin.cartlookscore.customers.details', ['id' => $customer->id]) }}"
        ...
        (function($) {
            ...
            $("#business_insight_change").on('change', function(e) {
                ...
                $.ajax({
                    ...
                    url: '{{ route('plugin.cartlookscore.business.stats') }}',
            ...
            function getChartData() {
                $.ajax({
                    ...
                    url: '{{ route('plugin.cartlookscore.reports.sales.chart') }}',
