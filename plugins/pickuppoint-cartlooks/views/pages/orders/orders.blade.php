        ...
        <div class="col-12">
            <div class="card mb-30">
                ...
                <div class="px-2 filter-area d-flex align-items-center mb-20">
                    ...
                    <a href="{{ route('plugin.pickuppoint.orders', ['delivery_status' => config('cartlookscore.order_delivery_status.pending')]) }}"
                        class="btn sm btn-primary">{{ translate('Pending') }}
                        ({{ $order_counter != null ? $order_counter['pending'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.pickuppoint.orders', ['delivery_status' => config('cartlookscore.order_delivery_status.processing')]) }}"
                        class="btn sm btn-info">{{ translate('Processing') }}
                        ({{ $order_counter != null ? $order_counter['processing'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.pickuppoint.orders', ['delivery_status' => config('cartlookscore.order_delivery_status.shipped')]) }}"
                        class="btn sm">{{ translate('To Shipped') }}
                        ({{ $order_counter != null ? $order_counter['shipped'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.pickuppoint.orders', ['delivery_status' => config('cartlookscore.order_delivery_status.cancelled')]) }}"
                        class="btn sm btn-danger">{{ translate('Cancelled') }}
                        ({{ $order_counter != null ? $order_counter['cancelled'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.pickuppoint.orders', ['delivery_status' => config('cartlookscore.order_delivery_status.delivered')]) }}"
                        class="btn sm btn-success">{{ translate('Delivered') }}
                        ({{ $order_counter != null ? $order_counter['delivered'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.pickuppoint.orders', ['payment_status' => config('cartlookscore.order_payment_status.unpaid')]) }}"
                        class="btn sm btn-warning">{{ translate('Unpaid') }}
                        ({{ $order_counter != null ? $order_counter['unpaid'] : 0 }})
                    </a>
                    <a href="{{ route('plugin.pickuppoint.orders', ['payment_status' => config('cartlookscore.order_payment_status.paid')]) }}"
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($order->customer_name != null)
                                                <a href="{{ route('plugin.cartlookscore.customers.details', ['id' => $order->customer_id]) }}"
                                                    target="_blank">
                                                    {{ $order->customer_name }}
                                                </a>
                                            @else
