...
@section('main_content')
    <form class="form-horizontal mb-4" action="{{ route('core.blog.comment.update') }}" method="post"
            ...
            <div class="col-md-8">
                <div class="card mb-30">
                    ...
                    <div class="card-body">
                        ...
                        {{-- Author --}}
                        <div class="my-1">
                            <h5 class="mb-20">{{ translate('Author Info') }}</h5>
                            <div class="row mb-2">
                            ...
                            <div class="form-row">
                                <h5 class="mb-20">{{ translate('Comment') }}</h5>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mx-1" id="visibility_form">
                            <input type="radio" checked name="status" id="status-radio-approve" value="approve"
                                {{ $comment->status == '1' ? 'checked' : '' }} />
                            <label for="status-radio-approve">{{ translate('Approve') }}</label>
                            <br />

                            <input type="radio" name="status" id="status-radio-pending" value="pending"
                                {{ $comment->status == '2' ? 'checked' : '' }} />
                            <label for="status-radio-pending">{{ translate('Pending') }}</label>
                            <br />

                            <input type="radio" name="status" id="status-radio-spam" value="spam"
                                {{ $comment->status == '3' ? 'checked' : '' }} /> <label
                        ...
                        <div class="row my-2 mx-1">
                            ...
                            <a href="{{ route('core.edit.blog', ['id' => $comment->blog_id, 'lang' => getDefaultLang()]) }}"
                                target="_blank"
                                class="text-primary d-block mb-1">{{ $comment->blog->translation('name', getLocale()) }}</a>
                        </div>
